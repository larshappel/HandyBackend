...
## 1. 背景
- ハンディ端末から読み取ったデータを直接データベースに書くよりも、専用のサーバーで受けて処理する理由は「業務ルールの整理」と「送信の証跡を残す」ためです。  
- 具体的には、同じ商品を何度も送ったときに重複して更新しない、存在しない商品を送信したときにエラーを返す、誰が何時送ったかを記録してあとから調査できるようにする必要があります。  
- こうした業務上の振る舞いをすべて守るには、バーコードの受信→判定→ログという手順をまとめて行う中間層（バックエンド）が必要で、Magic XPA で表示されない問題の原因追跡にも役立ちます。  
